...
class Binance(RESTInterface):
    ...
    def request(self, verb, endpoint, authenticate=False, **req_kwargs):
        ...
        return super(Binance, self).request(verb, endpoint,
    ...
    def _place_order(self, pair, price, size, side, *args, **kwargs):
        payload = {'symbol': pair,
                   ...
                   'quantity': size}
        payload.update(kwargs)
        return self.request('POST', 'v3/order', authenticate=True,
                            params=payload)
    ...
    def order_status(self, pair, order_id, *args, **kwargs):
        ...
        payload = {'symbol': pair, 'orderId': order_id}
        payload.update(kwargs)
        return self.request('GET', 'v3/order', authenticate=True,
                            params=payload)

    def open_orders(self, *args, **kwargs):
        """Return all open orders."""
        return self.request(
            'GET', 'v3/openOrders', authenticate=True, params=kwargs)
    ...
    def cancel_order(self, pair, *order_ids, **kwargs):
        ...
        for order_id in order_ids:
            payload = {'symbol': pair, 'orderId': order_id}
            payload.update(kwargs)
            r = self.request('DELETE', 'v3/order', authenticate=True,
                             params=payload)
            results.append(r)
